...
const eqObjects = function(object1, object2) {
  ...
  for (const key of keys) {
    // If both values are arrays and are equal, continue to next key (skip further checks)
    if (Array.isArray(object1[key]) && eqArrays(object1[key], object2[key])) {
      continue;
    }
    
    // If both values are objects and not arrays, continue to next key if eqObjects returns true on them
